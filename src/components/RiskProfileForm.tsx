...
import { createRiskProfileAction, type FormState } from "@/app/actions";
...
export function RiskProfileForm({ clientId }: { clientId: string }) {
  const [state, action] = useActionState(createRiskProfileAction, initial);
          ...
          {rows.map((row, i) => (
            <div key={i} className="flex gap-2">
              <input
                className={inputClass}
                name="attrKey"
                placeholder="Field (e.g. Vehicle VIN)"
                value={row.key}
