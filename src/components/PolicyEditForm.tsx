...
import { updatePolicyAction, type FormState } from "@/app/actions";
...
export function PolicyEditForm(props: Props) {
  ...
  const [rows, setRows] = useState<CovRow[]>(
    props.coverages.length > 0 ? props.coverages : [{ name: "", limit: "", deductible: "" }],
  );
  ...
  return (
    <form action={action} className="space-y-4 rounded-md border border-white/10 p-4">
      <input type="hidden" name="policyId" value={props.policyId} />
      ...
      <div className="grid grid-cols-2 gap-4">
        <div>
          ...
          <input className={inputClass} id="carrier" name="carrier" defaultValue={props.carrier} required />
        </div>
        <div>
          ...
          <input
            ...
            id="policyNumber"
            name="policyNumber"
            defaultValue={props.policyNumber}
            required
          />
        </div>
      </div>
      ...
      <div className="grid grid-cols-3 gap-4">
        <div>
          ...
          <select
            ...
            id="lineOfBusiness"
            name="lineOfBusiness"
            defaultValue={props.lineOfBusiness}
          >
        ...
        <div>
          ...
          <input className={inputClass} id="premium" name="premium" defaultValue={props.premium} />
        </div>
        <div>
          <label className={labelClass} htmlFor="status">
            Status
          </label>
          <select className={inputClass} id="status" name="status" defaultValue={props.status}>
