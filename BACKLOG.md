...
- [x] **Sales pipeline tracker** — `Deal` model + `PipelineStage` (qualified, quoting,
      proposed, won, lost, circle back); compact drag-and-drop board at `/pipeline`.
      Each deal has premium + commission % → **revenue** (the broker's commission, the
      key figure), plus probability + next action; per-stage revenue total + count.
      Adjustable monthly / quarterly / annual **revenue goals** with a "how we stand"
      view (won revenue per period vs. goal; `wonAt` stamped on the WON transition).
      Gmail ingestion seam (`POST
